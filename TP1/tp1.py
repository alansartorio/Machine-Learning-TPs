...
def calculate_predicate_probability_given(
    df: pd.DataFrame,
    predicate: Callable[[pd.Series], bool],
    given_var: str,
    given_value: str,
    class_amount=None,
    in_class_amount=None,
):
...
def calculate_probability_given(
    df: pd.DataFrame, var: str, value: str, given_var: str, given_value: str
):
    return calculate_predicate_probability_given(
        df, lambda r: r[var] == value, given_var, given_value
    )


def calculate_probability_of_being_in_class(
    var_probability: Dict[str, Dict[str, float]],
...
def classify_and_get_probabilities(
    var_probability: Dict[str, Dict[str, float]],
    class_probability: Dict[str, float],
    values: Dict[str, float],
):
    ...
    probabilities = {
        c: calculate_probability_of_being_in_class(
            var_probability, class_probability, values, c
        )
        for c in classes
    }
...
def build_var_probability(
    df: pd.DataFrame, variables: List[str], class_var: str, class_values: List[str]
):
...
def get_word_probability(
    word: str, clazz: str, df: DataFrame, class_amount: int, in_class_amount: int
):
    return calculate_predicate_probability_given(
        df,
        lambda r: word in r["data"],
        "categoria",
        clazz,
        class_amount,
        in_class_amount,
...
def classify_news(df: DataFrame, evaluation: DataFrame, vocabulary: set) -> DataFrame:
    ...
    for clazz in tqdm(classes):
        ...
        for word in tqdm(vocabulary_list):
            var_probabilities[clazz][word] = get_word_probability(
                word, clazz, df, class_amount, in_class_amount
            )
    ...
    for index, doc in evaluation.iterrows():
        prediction, probabilities = classify_and_get_probabilities(
            var_probabilities, class_probability, doc[vocab_vec_name]
        )
        data_dict = {
            "index": [index],
            "expected": [doc["categoria"]],
            "prediction": [prediction],
            "probability_prediction": [probabilities[prediction]],
        }
...
def preference_count_by_nationality(
    df: DataFrame, nationalities: Set, nationality_name: str, variables: Set
...
def part_1_pre_analysis(df: DataFrame, class_name: str, classes: Set, variables: Set):
    ...
    rel_pref = {
        k: {k2: v2 / count_by_nationality[k] for k2, v2 in v.items()}
    ...
    def double_barplot(
        ...
        res_df_long = res_df.melt(
            id_vars=x_title, var_name="nationality", value_name=y_title
        )
...
def part_1(df: DataFrame):
    ...
    def plot(prob, name):
        res = DataFrame(
            {f'P({"Ingles" if k == "I" else "Escoces"})': [v] for k, v in prob.items()}
        )
        print(res)
    ...
    class_x1, probabilities_x1 = classify_and_get_probabilities(
        var_probability, class_probability, x1
    )
    ...
    class_x2, probabilities_x2 = classify_and_get_probabilities(
        var_probability, class_probability, x2
    )
...
def part_2_pre_analysis(
    df: DataFrame, class_name: str, data_name: str, print_output=True, save_output=True
):
    ...
    top_words = DataFrame(
        list(sorted(bog.items(), key=lambda x: x[1], reverse=True)[:30]),
...
def part_2(df):
    ...
    class_name = "categoria"
    data_name = "titular"
    df = part_2_pre_analysis(
        df, class_name, data_name, print_output=False, save_output=True
    )
...
def part_3(df):
    ...
    def calculate_probability_given_list_of_vars(
        df: pd.DataFrame,
        var: str,
        value: int,
        given_vars: list[str],
        given_values: list[int],
    ):
        in_class = df
    ...
    for a in (1, 2, 3, 4):
        vars_probability["gre"][(a,)] = calculate_probability_given_list_of_vars(
            clean_df, "gre", 1, ["rank"], [a]
        )
        vars_probability["gpa"][(a,)] = calculate_probability_given_list_of_vars(
            clean_df, "gpa", 1, ["rank"], [a]
        )

    vars_probability["rank"][()] = build_class_probability(
        clean_df, "rank", [1, 2, 3, 4]
    )

    for a in (0, 1):
        for b in (0, 1):
            for c in (1, 2, 3, 4):
                vars_probability["admit"][(a, b, c)] = (
                    calculate_probability_given_list_of_vars(
                        clean_df, "admit", 1, ["gre", "gpa", "rank"], [a, b, c]
                    )
                )
    ...
    def calculate_intersection_probability(
        vars_intersecting: list[str], values: list[int]
    ):
        not_intersecting = [
            var for var in vars_probability.keys() if var not in vars_intersecting
        ]
        ...
        for vars in product(
            *[
                [(var, val) for val in var_possible_values[var]]
                for var in not_intersecting
            ]
        ):
            var_values = dict()
    ...
    a = calculate_intersection_probability(
        ["admit", "rank"], [0, 1]
    ) / calculate_intersection_probability(["rank"], [1])
    print(f"a) {a}")
    ...
    b = calculate_intersection_probability(
        ["admit", "rank", "gre", "gpa"], [1, 2, 1, 1]
    ) / calculate_intersection_probability(["rank", "gre", "gpa"], [2, 1, 1])
    print(f"b) {b}")
