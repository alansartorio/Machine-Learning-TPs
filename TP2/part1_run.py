...
class Forest:
    ...
    def train(
        self,
        tree_count: int,
        subset_generator: Callable[[int], DataFrame],
        value_mapping: Dict[str, Dict[int, str]],
...
if __name__ == "__main__":
    ...
    def reduce_column(df: pl.DataFrame, column, factor):
        ...
        value_mapping[column] = {
            reduced: f"[{reduced * factor}, {(reduced + 1) * factor})"
            for reduced in series.unique()
        }
    ...
    def single_tree():
        ...
        for tree_depth in range(1, 11):
            ...
            print()
            evaluation_results = evaluate_model(evaluation, forest).with_columns(
                pl.lit("evaluation").alias("data split")
            )
            training_results = evaluate_model(training, forest).with_columns(
                pl.lit("training").alias("data split")
            )
            results = pl.concat([evaluation_results, training_results]).with_columns(
        ...
        all_results = pl.concat(all_results, rechunk=True).with_columns(
            [pl.lit(1).alias("tree count"), pl.lit(len(training)).alias("bag size")]
        )
    ...
    def single_tree_for_graph():
        forest = train_model(
            training, value_mapping, subset_size=None, tree_count=1, max_depth=2
        )
    ...
    def forest():
        ...
        complete_configs = list(
            (training.clone(), value_mapping.copy(), *config) for config in configs
        )
        # print(complete_configs)
        main_forest = None

        # with Pool(12) as pool:
        for forest, (tree_count, subset_size, tree_depth) in tqdm(
            zip(map(train, complete_configs), configs), total=len(configs)
        ):
            # forest = train_model(training, value_mapping, subset_size=subset_size, tree_count=tree_count, max_depth=tree_depth)
            # print()
            if (
                tree_count == best_tree_count
                and subset_size == best_subset_size
                and tree_depth == best_tree_depth
            ):
                main_forest = forest
            evaluation_results = evaluate_model(evaluation, forest).with_columns(
                pl.lit("evaluation").alias("data split")
        ...
        DataFrame({"first attribute": first_attributes}).write_csv(
            "out/part1_first_attributes.csv"
        )
