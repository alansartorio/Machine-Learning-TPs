...
def plot_distribution_for_variable(df: DataFrame, variable: str, config: PlotConfig, after: Optional[Callable] = None, type: DistType = DistType.HIST, xticks: Optional[Iterable] = None, xtick_labels: Optional[Iterable[str]] = None, **kwargs):
    column = df.get_column(variable)
    match type:
        case DistType.HIST:
            ax = sns.histplot(df, x=variable, kde=False, hue=creditability, multiple="stack", **kwargs)
        case DistType.BOXPLOT:
            ax = sns.boxplot(column, **kwargs)
        case DistType.COUNTPLOT:
            ax = sns.histplot(df, x=variable, kde=False, hue=creditability, multiple="stack", discrete=True, **kwargs)
...
def do_thing(df, column):
    return df \
        .group_by([pl.col(column), pl.col(creditability)]) \
        .count() \
        .pivot(index=column, columns=creditability, values='count') \
        .fill_null(0) \
        .with_columns((pl.col('1') / (pl.col('1') + pl.col('0'))).alias('1')) \
        .melt(id_vars=column, value_vars=['1'], variable_name=creditability, value_name='percentage')
...
if __name__ == '__main__':
    ...
    credit_df = do_thing(df.with_columns((pl.col(credit_amount) // 500) * 500), credit_amount)
    plot_distribution_for_variable(
        credit_df,
        credit_amount,
