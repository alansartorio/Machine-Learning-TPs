...
def load_image(
    filepath: Path, get_class: Callable[[int, int, int, int, int], UInt8]
) -> DataFrame:
    """
...
def plot_rgb_projections(
    df: DataFrame, title: str, show: bool = True, save_path: Optional[Path] = None
):
    ...
    axs[0].scatter(
        data_normalized[:, 0], data_normalized[:, 1], c=data_normalized, s=100
    )
    ...
    axs[1].scatter(
        data_normalized[:, 1], data_normalized[:, 2], c=data_normalized, s=100
    )
    ...
    # Blue-Red projection
    axs[2].scatter(
        data_normalized[:, 2], data_normalized[:, 0], c=data_normalized, s=100
    )
...
def paint_image(
    ...
    classifications = classifications.reshape(
        (image_pixels.shape[0], image_pixels.shape[1])
    )
