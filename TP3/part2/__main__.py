...
def print_class_sizes(df: DataFrame):
    print(
        "Vaca:",
        f"{df.filter(pl.col('class') == CLASSES['vaca']).select(pl.len()).item():,}",
    )
    print(
        "Cielo:",
        f"{df.filter(pl.col('class') == CLASSES['cielo']).select(pl.len()).item():,}",
    )
    print(
        "Pasto:",
        f"{df.filter(pl.col('class') == CLASSES['pasto']).select(pl.len()).item():,}",
    )
...
def generate_rgb_cubes():
    plot_rgb_cube(
        ...
        save_path=plot_file("cow_rgb.svg"),
    )
    plot_rgb_projections(
        data_frames["cow"],
        "Proyecciones cow.jpg",
        show=config["show_plot"],
        save_path=plot_file("cow_rgb_projections.svg"),
    )
    plot_rgb_cube(
        data_frames["vaca"],
        "vaca.jpg",
        show=config["show_plot"],
        save_path=plot_file("vaca_rgb.svg"),
    )
    plot_rgb_projections(
        data_frames["vaca"],
        "Proyecciones vaca.jpg",
        show=config["show_plot"],
        save_path=plot_file("vaca_rgb_projections.svg"),
    )
    plot_rgb_cube(
        data_frames["pasto"],
        "pasto.jpg",
        show=config["show_plot"],
        save_path=plot_file("pasto_rgb.svg"),
    )
    plot_rgb_projections(
        data_frames["pasto"],
        "Proyecciones pasto.jpg",
        show=config["show_plot"],
        save_path=plot_file("pasto_rgb_projections.svg"),
    )
    plot_rgb_cube(
        data_frames["cielo"],
        "cielo.jpg",
        show=config["show_plot"],
        save_path=plot_file("cielo_rgb.svg"),
    )
    plot_rgb_projections(
        data_frames["cielo"],
        "Proyecciones cielo.jpg",
        show=config["show_plot"],
        save_path=plot_file("cielo_rgb_projections.svg"),
    )
...
classify_image(
    model, input_file("vacas_1.jpg"), plot_file("classified_vacas_1.jpg"), False
)
classify_image(
    model, input_file("vacas_2.jpg"), plot_file("classified_vacas_2.jpg"), False
)
classify_image(
    model, input_file("vacas_3.jpg"), plot_file("classified_vacas_3.jpg"), False
)
...
data = [
    ...
    for r, g, b, clazz in train.with_columns(
